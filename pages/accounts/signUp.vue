...
useHead({
  script: [{ src: 'https://static.nid.naver.com/js/naveridlogin_js_sdk_2.0.2.js', tagPosition: 'head' }],
});
...
function getNaverAuthorizUrl() {
  const config = useRuntimeConfig();
  const params = {
    response_type: 'code',
    state: uuidv1(),
    client_id: config.public.naver_client_id,
    redirect_uri: config.public.naver_authoriz_callback_url,
  };
  return _.join(
    [
      config.public.naver_authoriz_url,
      _.join(
        _.map(params, (v, k) => _.join([k, v], '=')),
        '&'
      ),
    ],
    '?'
...
function getKakaoAuthorizUrl() {
  const config = useRuntimeConfig();
  const params = {
    response_type: 'code',
    state: uuidv1(),
    client_id: config.public.kakao_client_id,
    redirect_uri: config.public.kakao_authoriz_callback_url,
  };
  return _.join(
    [
      config.public.kakao_authoriz_url,
      _.join(
        _.map(params, (v, k) => _.join([k, v], '=')),
        '&'
      ),
